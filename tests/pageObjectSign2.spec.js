...
const { loginpage2 } = require('../pageobject/Loginpage2')




test('TC', async function ({ browser }) {
    const context = await browser.newContext();
    const page = await context.newPage();
    const username = "vrt@gg2.com";
    const password = "@Karen23";
    const logpage = new loginpage2(page);
    await logpage.goTO();
    await logpage.validsign(username,password)
